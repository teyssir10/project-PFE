...
export default function SettingsCard({ question, onUpdate }: SettingsCardProps) {
  ...
  const POINTS_OPTIONS = [
    { value: "No points", label: t("noPoints"), icon: "—", desc: t("noPointsDesc") },
    { value: "Standard (1x)", label: t("standard"), icon: "★", desc: t("standardDesc") },
    { value: "Double (2x)", label: t("double"), icon: "★★", desc: t("doubleDesc") },
  ];
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {POINTS_OPTIONS.map((opt) => {
            ...
            return (
              <button key={opt.value} type="button" onClick={() => onUpdate({ points: opt.value })}
                ...
                <span className={`text-lg mb-1 ${isActive ? "text-white" : "text-amber-400"}`}>{opt.icon}</span>
