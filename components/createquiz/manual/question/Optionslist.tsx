...
export default function OptionsList({ options, correctOptionId, questionType, onSelectCorrect, onUpdateOption, onDeleteOption, onAddOption }: OptionsListProps) {
  const t = useTranslations("optionsList");

  return (
    <div>
      <label className="block text-[10px] font-bold text-gray-400 dark:text-slate-500 tracking-widest uppercase mb-3">
        {t("answersLabel")}{" "}
        <span className="normal-case font-normal">{t("answersHint")}</span>
      </label>
      ...
      <div className="space-y-2">
        {options.map((opt, optIdx) => {
          ...
          return (
            <div key={opt.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-150 ${
              isCorrect
              ...
              <span className={`text-[10px] font-bold w-5 h-5 rounded flex items-center justify-center shrink-0 ${
                isCorrect ? "bg-cyan-500 text-white" : "bg-gray-200 dark:bg-slate-700 text-gray-400 dark:text-slate-500"
              }`}>
                {OPTION_LETTERS[optIdx]}
              </span>

              <button onClick={() => onSelectCorrect(opt.id)}
                className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                  isCorrect ? "border-cyan-500 bg-cyan-500" : "border-gray-300 dark:border-slate-600 hover:border-cyan-400"
                }`}>
                {isCorrect && <span className="w-1.5 h-1.5 rounded-full bg-white" />}
              </button>

              <input type="text" value={opt.text} onChange={(e) => onUpdateOption(opt.id, e.target.value)}
                placeholder={t("optionPlaceholder", { letter: OPTION_LETTERS[optIdx] })}
