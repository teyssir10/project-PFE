...
export default function ManualQuiz() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await saveQuiz({ quizTitle, questions, userId: user.id, userFirstname: user.user_metadata?.firstname, userLastname: user.user_metadata?.lastname, quizDifficulty: difficulty, coverImage, getEffectiveTimeLimit });
    ...
    <>
      <div className="min-h-screen pb-14 bg-gray-50 dark:bg-slate-950 font-sans flex transition-colors duration-300">
        <QuizSidebar
          ...
          onSelectQuestion={setActiveId} onAddQuestion={addQuestion} onSave={handleSave} onTitleChange={setQuizTitle}
        />
      ...
      <StepperBar currentStep={2} withSidebar steps={MANUAL_STEPS} onPublish={handleSave} />
    </>
  );
}